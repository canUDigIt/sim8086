...
pub fn main() anyerror!void {
    ...
    while (buf_counter + wanted_bytes <= buf_size) : (buf_counter += wanted_bytes) {
        ...
        if (short_op == 0b1011) {
            ...
            const immediate = if (word) std.mem.bytesToValue(u16, buf_ptr[1..3]) else buf_ptr[1];
            wanted_bytes = if (word) 3 else 2;
            ...
            if (mod == 0b11) {
                ...
                if (mod == 0b00 and reg_or_memory == 0b110) {
                    wanted_bytes = 4;
                    rm_name = try std.fmt.bufPrint(&string_buffer, "{d}", .{std.mem.bytesToValue(u16, buf_ptr[2..4])});
                } else {
                    ...
                    if (mod == 0b00) {
                        ...
                        const disp = if (disp_8bit) buf_ptr[2] else std.mem.bytesToValue(u16, buf_ptr[2..4]);
                        wanted_bytes = if (disp_8bit) 3 else 4;
